...
public int GetClosestDistance(List<Movement> inputA, List<Movement> inputB) {
    ...
    var visitedByBoth = visitedByA.Where(x => visitedByB.Contains(x)).ToList().Skip(1);

    var distanceSumsForEarliestIntersection = visitedByBoth.Select(x => visitedByA.IndexOf(x) + visitedByB.IndexOf(x)).Min();
    Console.WriteLine(distanceSumsForEarliestIntersection);
    return distanceSumsForEarliestIntersection;
}
